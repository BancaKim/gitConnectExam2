...
public class ArrayExam3 {
	...
	public static void main(String[] args) {
		...
		int num = ArrayExam3.personNum(sc); // 사람 수 입력
		...
		for (int i = 0; i < num; i++) {
			name[i] = ArrayExam3.nameInput(sc); // 이름 입력

			for (int j = 0; j < score[i].length - 1; j++) {
				score[i][j] = ArrayExam3.scoreInput(sc); // 점수 입력
				score[i][3] += score[i][j];
			}
		}
		// 연산 : 평균
		...
		ArrayExam3.display(num, name, score, avg);
